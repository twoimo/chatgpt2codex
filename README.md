...
The macOS package requires a real Apple code-signing identity and team-scoped keychain entitlements for the Secure Enclave helper; ad-hoc signing is refused for write-capable packaging. Local development may use an Apple Development Personal Team identity with an Xcode-generated provisioning profile via `CODESIGN_IDENTITY`, `CODESIGN_TEAM_ID`, and `PROVISIONING_PROFILE_PATH`; the profile must authorize the helper's keychain group.
